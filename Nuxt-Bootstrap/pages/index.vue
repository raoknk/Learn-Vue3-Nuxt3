<template>
  <img src="~/assets/bg.jpg" />
  <section
    data-bs-version="5.1"
    class="menu menu1 growthm5 cid-tgzsba4rsv"
    once="menu"
    id="amenu1-0"
  >
    <nav class="navbar navbar-dropdown navbar-fixed-top navbar-expand-lg">
      <div class="container">
        <div class="navbar-brand">
          <span class="navbar-caption-wrap">
            <a
              class="navbar-caption text-black display-4"
              href="https://mobiri.se"
            >
              GrowthM5
            </a></span
          >
        </div>
        <button
          class="navbar-toggler"
          type="button"
          data-toggle="collapse"
          data-bs-toggle="collapse"
          data-target="#navbarSupportedContent"
          data-bs-target="#navbarSupportedContent"
          aria-controls="navbarNavAltMarkup"
          aria-expanded="false"
          aria-label="Toggle navigation"
        >
          <div class="hamburger">
            <span></span>
            <span></span>
            <span></span>
            <span></span>
          </div>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          <ul class="navbar-nav nav-dropdown" data-app-modern-menu="true">
            <li class="nav-item">
              <a
                class="nav-link link text-black display-7"
                href="https://mobiri.se"
              >
                About
              </a>
            </li>
            <li class="nav-item">
              <a
                class="nav-link link text-black display-7"
                href="https://mobiri.se"
              >
                Work
              </a>
            </li>
            <li class="nav-item">
              <a
                class="nav-link link text-black display-7"
                href="https://mobiri.se"
              >
                Blog
              </a>
            </li>
            <li class="nav-item">
              <a
                class="nav-link link text-black display-7"
                href="https://mobiri.se"
              >
                Contact
              </a>
            </li>
          </ul>

          <div class="navbar-buttons mbr-section-btn">
            <a class="btn btn-warning display-7" href="https://mobiri.se">
              We're hiring</a
            >
          </div>
        </div>
      </div>
    </nav>
  </section>

  <section
    data-bs-version="5.1"
    class="header2 growthm5 cid-tgzscJ4ACx mbr-fullscreen"
    id="aheader2-1"
  >
    <div
      class="mbr-overlay"
      style="opacity: 0; background-color: rgb(237, 122, 95)"
    ></div>

    <div class="container">
      <div class="row justify-content-center">
        <div class="col-12 col-lg-8 main-col">
          <h1 class="mbr-section-title mbr-fonts-style display-4">
            About GrowthM5
          </h1>
          <p class="mbr-text mbr-fonts-style mb-0 display-5">
            Scelerisque varius morbi enim<br />
            nunc faucibus a pellentesque.
          </p>
          <div class="mbr-section-btn">
            <a class="btn btn-warning display-7" href="https://mobiri.se">
              Contact Us
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="features4 growthm5 cid-tgztbFTYYF"
    id="afeatures4-3"
  >
    <div class="container">
      <div class="row">
        <div class="col-text">
          <div class="text-wrapper">
            <h1 class="mbr-section-title mbr-fonts-style display-5">
              Customize Your Logo with 'Creative Logo Studio'
            </h1>
            <p class="mbr-text mbr-fonts-style display-7">
              Auctor urna nunc id cursus metus aliquam eleifend. Dolor sit amet
              consectetur adipiscing elit pellentesque habitant morbi tristique.
              Nunc faucibus a pellentesque sit amet. Morbi blandit cursus risus
              at ultrices mi. Consequat mauris nunc congue nisi vitae suscipit
              tellus mauris. Pharetra diam sit amet nisl suscipit adipiscing
              bibendum est ultricies. Malesuada proin libero nunc consequat
              interdum.
            </p>
          </div>
        </div>
        <div class="col-img">
          <div class="image-wrapper">
            <img src="assets/images/1.jpg" alt="Mobirise Website Builder" />
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="people2 growthm5 cid-tgzu29NO9u"
    id="apeople2-4"
  >
    <div class="container">
      <div class="row item-row">
        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/pexels-ron-lach-7774246.jpeg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <h5 class="item-title mbr-fonts-style align-center display-7">
                Logo creation
              </h5>
              <p class="mbr-text mbr-fonts-style mb-0 align-center display-7">
                Facilisis gravida neque convallis a cras semper auctor. Faucibus
                et molestie ac feugiat sed lectus. Fermentum posuere urna nec
                tincidunt praesent semper feugiat.
              </p>
            </div>
          </div>
        </div>

        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/pexels-christina-morillo-1181263.jpeg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <h5 class="item-title mbr-fonts-style align-center display-7">
                Website Design &amp; Development
              </h5>
              <p class="mbr-text mbr-fonts-style mb-0 align-center display-7">
                Facilisis gravida neque convallis a cras semper auctor. Faucibus
                et molestie ac feugiat sed lectus. Fermentum posuere urna nec
                tincidunt praesent semper feugiat.
              </p>
            </div>
          </div>
        </div>
        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/pexels-fauxels-3182773.jpeg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <h5 class="item-title mbr-fonts-style align-center display-7">
                Apps Design &amp; Development
              </h5>
              <p class="mbr-text mbr-fonts-style mb-0 align-center display-7">
                Facilisis gravida neque convallis a cras semper auctor. Faucibus
                et molestie ac feugiat sed lectus. Fermentum posuere urna nec
                tincidunt praesent semper feugiat.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="features10 growthm5 cid-tgzvWjh2Pa"
    id="afeatures10-5"
  >
    <div class="container">
      <div class="row">
        <div class="col-12 col-lg-6 content-container">
          <p class="mbr-text mbr-fonts-style mb-0 display-5">
            When the looks matter –<br />
            we deliver the best
          </p>
          <div class="image-wrapper">
            <img
              src="assets/images/pexels-duophenom-2417848.jpeg"
              alt="Mobirise Website Builder"
            />
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="features5 growthm5 cid-tgzxd9AYXh"
    id="afeatures5-6"
  >
    <div class="container">
      <div class="row">
        <div class="col-img">
          <div class="image-wrapper">
            <img
              src="assets/images/undraw-lightbulb-moment-re-ulyo.svg"
              alt="Mobirise Website Builder"
            />
          </div>
        </div>
        <div class="col-text">
          <div class="text-wrapper">
            <h1 class="mbr-section-title mbr-fonts-style display-7">
              What we value
            </h1>
            <h1 class="mbr-section-subtitle mbr-fonts-style display-5">
              Good Logo design ensures a better customer response
            </h1>
            <p class="mbr-text mbr-fonts-style display-7">
              Ipsum suspendisse ultrices gravida dictum fusce ut placerat orci
              nulla velit ut tortor pretium viverra suspendisse.
            </p>
            <div class="mbr-section-btn">
              <a class="btn btn-warning display-7" href="https://mobiri.se"
                >Live Chat</a
              >
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="article1 growthm5 cid-tgzxDsoppJ"
    id="aarticle1-8"
  >
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-md-12 col-lg-12">
          <p class="mbr-text mbr-fonts-style mb-0 display-5">
            <strong>Our Recent awesome work</strong>
          </p>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="people1 growthm5 cid-tgzxC5Hc5D"
    id="apeople1-7"
  >
    <div class="images-container container">
      <div class="row justify-content-center">
        <div class="card">
          <img src="assets/images/1.png" alt="Mobirise Website Builder" />
        </div>
        <div class="card">
          <img src="assets/images/2.png" alt="Mobirise Website Builder" />
        </div>
        <div class="card">
          <img src="assets/images/3.png" alt="Mobirise Website Builder" />
        </div>
        <div class="card">
          <img src="assets/images/4.png" alt="Mobirise Website Builder" />
        </div>
        <div class="card">
          <img src="assets/images/5.png" alt="Mobirise Website Builder" />
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="list2 growthm5 cid-tgzAeIVW1k"
    id="alist2-f"
  >
    <div class="container">
      <div class="row">
        <div class="col-12 col-lg-6 content-container">
          <div class="title-container col-12">
            <h5 class="mbr-section-title mbr-fonts-style mb-0 display-5">
              <strong>How It Works:</strong>
            </h5>
          </div>
          <div class="tabl-container">
            <div class="item-container">
              <p class="card-text mbr-fonts-style mb-0 display-4">
                <strong> 1.</strong> Brainstorm Idea
              </p>
            </div>

            <div class="item-container">
              <p class="card-text mbr-fonts-style mb-0 display-4">
                <strong> 2.</strong> Logo Design Development
              </p>
            </div>
            <div class="item-container">
              <p class="card-text mbr-fonts-style mb-0 display-4">
                <strong> 3.</strong> Review, Feedback, and Revisions
              </p>
            </div>
            <div class="item-container">
              <p class="card-text mbr-fonts-style mb-0 display-4">
                <strong> 4.</strong> Project Finalization
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="features3 growthm5 cid-tgzyBxE338"
    id="afeatures3-a"
  >
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-12 col-lg-12 main-col">
          <div class="content-container">
            <div class="img-bg-wrap"></div>
            <div class="text-wrap">
              <p class="mbr-text mbr-fonts-style mb-0 display-5">
                Our Clients Believe Our Creative Logo Design Services Are To Die
                For
              </p>
            </div>
            <div class="mbr-section-btn card-btns">
              <a class="btn btn-warning display-7" href="https://mobiri.se">
                Contact Us
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="features11 growthm5 cid-tgzxPimAOs"
    id="afeatures11-9"
  >
    <div class="container">
      <div class="row">
        <div class="col-12 col-lg-6 content-container">
          <h5 class="mbr-section-title mbr-fonts-style mb-0 display-5">
            <strong>Pocket-Friendly Price Packages</strong>
          </h5>
          <p class="mbr-text mbr-fonts-style mb-0 display-7">
            Mattis enim ut tellus elementum sagittis vitae et. Leo in vitae
            turpis massa. Arcu cursus vitae congue mauris rhoncus.
          </p>
          <div class="image-wrapper">
            <img
              src="assets/images/screenshot-3.png"
              alt="Mobirise Website Builder"
            />
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="gallery1 growthm5 cid-tgzzHL4utG"
    id="agallery1-d"
  >
    <div class="container">
      <div class="mbr-section-head">
        <h5 class="mbr-section-subtitle mbr-fonts-style display-5">
          Latest blog posts
        </h5>
      </div>
      <div class="row item-row">
        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/gallery1.jpg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <div class="item-text-wrap">
                <h5 class="item-title mbr-fonts-style display-4">
                  Ornare arcu odio ut sem
                </h5>
              </div>
              <div class="card-bottom">
                <div class="item-name-wrap">
                  <div class="image-wrapper">
                    <img
                      src="assets/images/team1.jpg"
                      alt="Mobirise Website Builder"
                    />
                  </div>
                  <p class="mbr-text mbr-fonts-style mb-0 display-7">
                    Adriana Miller
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/gallery2.jpg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <div class="item-text-wrap">
                <h5 class="item-title mbr-fonts-style display-4">
                  Eget dolor morbi non arcu risus quis varius
                </h5>
              </div>
              <div class="card-bottom">
                <div class="item-name-wrap">
                  <div class="image-wrapper">
                    <img
                      src="assets/images/team2.jpg"
                      alt="Mobirise Website Builder"
                    />
                  </div>
                  <p class="mbr-text mbr-fonts-style mb-0 display-7">
                    Jenny Williams
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/gallery3.jpg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <div class="item-text-wrap">
                <h5 class="item-title mbr-fonts-style display-4">
                  Pharetra pharetra massa massa
                </h5>
              </div>
              <div class="card-bottom">
                <div class="item-name-wrap">
                  <div class="image-wrapper">
                    <img
                      src="assets/images/team3.jpg"
                      alt="Mobirise Website Builder"
                    />
                  </div>
                  <p class="mbr-text mbr-fonts-style mb-0 display-7">
                    Jennifer Davis
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/gallery4.jpg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <div class="item-text-wrap">
                <h5 class="item-title mbr-fonts-style display-4">
                  Dolor morbi non arcu risus quis fringilla
                </h5>
              </div>
              <div class="card-bottom">
                <div class="item-name-wrap">
                  <div class="image-wrapper">
                    <img
                      src="assets/images/team4.jpg"
                      alt="Mobirise Website Builder"
                    />
                  </div>
                  <p class="mbr-text mbr-fonts-style mb-0 display-7">
                    Ethan Johnson
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="people3 growthm5 cid-tgzzNrgrha"
    id="apeople3-e"
  >
    <div class="container">
      <div class="mbr-section-head">
        <h4 class="mbr-section-title mbr-fonts-style align-center display-5">
          <strong>Our team</strong>
        </h4>
      </div>
      <div class="row item-row">
        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/team5.jpg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <h5 class="item-title mbr-fonts-style align-center display-4">
                <strong>Adrian Walker</strong>
              </h5>
              <p class="mbr-text mbr-fonts-style mb-0 align-center display-4">
                CEO
              </p>
            </div>
          </div>
        </div>

        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/team6.jpg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <h5 class="item-title mbr-fonts-style align-center display-4">
                <strong>Alice Wood</strong>
              </h5>
              <p class="mbr-text mbr-fonts-style mb-0 align-center display-4">
                CTO
              </p>
            </div>
          </div>
        </div>
        <div class="item features-image сol-12">
          <div class="item-wrapper">
            <div class="item-img">
              <img
                src="assets/images/team7.jpg"
                alt="Mobirise Website Builder"
              />
            </div>
            <div class="item-content">
              <h5 class="item-title mbr-fonts-style align-center display-4">
                <strong>Jack Wright</strong>
              </h5>
              <p class="mbr-text mbr-fonts-style mb-0 align-center display-4">
                Developer
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="video1 growthm5 cid-tgzASWISmL"
    id="avideo1-g"
  >
    <div class="align-center container">
      <div class="row justify-content-center">
        <div class="col-12 col-md-11 col-lg-11 col-video">
          <div class="box">
            <div
              class="mbr-media show-modal align-center"
              data-modal=".modalWindow"
            >
              <img
                src="assets/images/pexels-olya-kobruseva-7598019.jpeg"
                alt="Mobirise"
              />
              <div class="icon-wrap">
                <span
                  class="mbr-iconfont mobi-mbri-play socicon mobi-mbri"
                  style="
                    color: rgb(39, 39, 39);
                    font-size: 32px;
                    fill: rgb(39, 39, 39);
                  "
                ></span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div>
      <div class="modalWindow" style="display: none">
        <div class="modalWindow-container">
          <div class="modalWindow-video-container">
            <div class="modalWindow-video">
              <iframe
                width="100%"
                height="100%"
                frameborder="0"
                allowfullscreen="1"
                data-src="https://www.youtube.com/watch?v=VA8PzkQa8d8"
              ></iframe>
            </div>
            <a
              class="close"
              role="button"
              data-dismiss="modal"
              data-bs-dismiss="modal"
            >
              <span
                aria-hidden="true"
                class="mbr-iconfont mobi-mbri-close mobi-mbri closeModal"
              ></span>
              <span class="sr-only visually-hidden">Close</span>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="form1 growthm5 cid-tgzz1HCKOQ"
    id="aform1-b"
  >
    <div class="container">
      <div class="row justify-content-center col-12 col-lg-11">
        <div class="col-title col-12">
          <h1 class="form-title mbr-fonts-style mb-0 display-5">Let's Talk?</h1>
        </div>
        <div class="col-form">
          <div class="form-wrap">
            <div class="mbr-form" data-form-type="formoid">
              <!--Formbuilder Form-->
              <form
                action="https://mobirise.eu/"
                method="POST"
                class="mbr-form form-with-styler mx-auto"
                data-form-title="Form Name"
              >
                <input
                  type="hidden"
                  name="email"
                  data-form-email="true"
                  value="MVQd7MagoH7WtzxUz6JmZ265Vsj0+yY+xmajrVElFCcEg2qd2nohQSq20CxC/KRoHjxwSDGBSU7iNPgdkkeI92WMU9lYEP8gfrUovQ7RnJfw6CHlfIlq/n8sdr28yTDO.b14sWAPI8dhD3NyOm9682peeR1WmRmGQRZ3IhjsDZyEoC+6xUwoyH3a96EMXZ/V1deBLbfX5tt9pPqOBwjE1NcySV+rE6iPyMVZyRS6nibFAIdhCgh7pWs0f/cWcoXRZ"
                />
                <div class="row">
                  <div
                    hidden="hidden"
                    data-form-alert=""
                    class="alert alert-success col-12"
                  >
                    Thanks for filling out the form!
                  </div>
                  <div
                    hidden="hidden"
                    data-form-alert-danger=""
                    class="alert alert-danger col-12"
                  >
                    Oops...! some problem!
                  </div>
                </div>
                <div class="dragArea row">
                  <div
                    class="col-lg-12 col-md-12 col-sm-12 form-group mb-3 mb-3 mb-3 mb-3"
                    data-for="name"
                  >
                    <input
                      type="text"
                      name="name"
                      placeholder="Name"
                      data-form-field="name"
                      required="required"
                      class="form-control display-2 display-7"
                      value=""
                      id="name-aform1-b"
                    />
                  </div>
                  <div
                    class="col-lg-12 col-md-12 col-sm-12 form-group mb-3"
                    data-for="message"
                  >
                    <input
                      type="text"
                      name="message"
                      placeholder="Message"
                      data-form-field="message"
                      class="form-control display-2 display-7"
                      value=""
                      id="message-aform1-b"
                    />
                  </div>
                  <div
                    data-for="phone"
                    class="col-lg-12 col-md-12 col-sm-12 form-group"
                  >
                    <input
                      type="text"
                      name="phone"
                      placeholder="Phone"
                      data-form-field="phone"
                      class="form-control display-7"
                      value=""
                      id="phone-aform1-b"
                    />
                  </div>
                  <div
                    class="col-lg-12 col-md-12 col-sm-12 form-group mb-3 mb-3 mb-3 mb-3 mb-3"
                    data-for="email"
                  >
                    <input
                      type="email"
                      name="email"
                      placeholder="Email"
                      data-form-field="email"
                      required="required"
                      class="form-control display-2 display-7"
                      value=""
                      id="email-aform1-b"
                    />
                  </div>
                  <div class="col-auto align-center mbr-section-btn">
                    <button
                      type="submit"
                      class="w-100 btn btn-secondary display-7"
                    >
                      Send
                    </button>
                  </div>
                </div>
              </form>
              <!--Formbuilder Form-->
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section
    data-bs-version="5.1"
    class="footer2 growthm5 cid-tgzzEZagk3"
    once="footers"
    id="afooter2-c"
  >
    <div class="container">
      <div class="media-container-row align-center mbr-white">
        <div class="col-12">
          <p class="mbr-text mb-0 mbr-fonts-style display-7">
            © Copyright 2030 Mobirise - All Rights Reserved
          </p>
        </div>
      </div>
    </div>
  </section>
</template>
